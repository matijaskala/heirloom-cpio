...
#if ENABLE_ZIP
static int	zipblast(struct file *f, const char *tgt, int tfd, int doswap, uint32_t *crc);
...
main(int argc, char **argv)
{
	...
	if (!memcmp(progname, "pax", 3))
		...
		cpio_flags(argc, argv);
	struct sigaction sa_old, sa = { .sa_handler = onint, .sa_flags = 0 };
	switch (action) {
	case 'i':
...
{
	return getline(name, namsiz, stdin);
}
	...
	while ((namlen = ofiles(&name, &namsiz)) != 0) {
		...
		if (lstat(np, &st) < 0) {
			if (Pflag && *np && ((pt.pt_spec &
				...
				warnx("Cannot obtain information "
						"about file:  \"%s\".",
						np);
			else
				warn("Error with lstat of \"%s\"", np);
			errcnt++;
			continue;
		}
	missingok:
...
dooutp(void)
{
	if (Oflag) {
		if ((mt = Aflag ? open(Oflag, O_RDWR, 0666) :
					creat(Oflag, 0666)) < 0) {
			if (sysv3) {
				warn("Cannot open <%s> for %s.", Oflag,
					Aflag ? "append" : "output");
				done(1);
			} else {
				warn("Cannot open \"%s\" for %s", Oflag,
...
outfile(const char *file, struct stat *st)
{
	...
	if (st->st_size > maxsize) {
		warnx("Size of %c%s%c >%lluGB. Not dumped",
				sysv3 ? '<' : '"',
				file,
				sysv3 ? '>' : '"',
	...
	if ((pathsz = strlen(file)) > maxpath) {
		warnx("%s: file name too long", file);
		return 1;
	}
	/*
...
nextdev:
		...
		if (dev >= fakedev) {
			warnx("Too many devices in archive, exiting");
			done(1);
		}
		if (ds == NULL) {
		...
		uint32_t dev, uint32_t ino, int zerolink, const char *linkname)
{
	...
	if (fmttype == FMT_PAX && pax_oflag & PO_LINKDATA &&
		...
		if ((size = readlink(realfile, symblink, i)) < 0) {
			warn("Cannot read symbolic link \"%s\"", realfile);
			goto cleanup;
		}
		symblink[size] = '\0';
	} else
	...
	if (fmttype & TYP_NCPIO) {
		...
		if (n > 512-SIZEOF_bar_header-fsz-1) {
			warnx("%s: linked name too long", realfile);
			goto cleanup;
		}
		bchksum(&bc);
		hsz = 512;
		...
		if ((c = tlflag(st)) < 0) {
			if ((st->st_mode&S_IFMT) != S_IFDIR) {
				warnx("%s is not a file. Not dumped",
						realfile);
				errcnt++;
			} else
				failure = 0;
		...
		if (fmttype & TYP_USTAR) {
			...
			if ((cp = getuser(st->st_uid)) != NULL)
				...
				warnx("could not get passwd information "
						"for %s", realfile);
			if ((cp = getgroup(st->st_gid)) != NULL)
				sprintf(bc.Tdr.t_gname, "%.31s", cp);
			else
				warnx("could not get group information "
						"for %s", realfile);
			if (fmttype == FMT_GNUTAR &&
...
setfmt(char *s)
{
	...
	warnx("Invalid header \"%s\" specified.", s);
	return -1;
}
...
setreassign(const char *s)
{
	...
	if (myuid != 0) {
		warnx("R option only valid for super-user.");
		val = -1;
	}
	if ((pwd = getpwnam(s)) == NULL) {
		warnx("\"%s\" is not a valid user id", s);
		val = -1;
	} else {
		Ruid = pwd->pw_uid;
		Rgid = pwd->pw_gid;
...
prdot(int flush)
{
	...
	if ((flush && column != 0) || column >= 50) {
		fputc('\n', action == 'o' && !Oflag ? stderr : stdout);
		column = 0;
	}
	if (!flush) {
		fputc('.', action == 'o' && !Oflag ? stderr : stdout);
		column++;
	}
}
...
newmedia(int err)
{
	...
	if (((mtst.st_mode&S_IFMT)!=S_IFCHR && (mtst.st_mode&S_IFMT)!=S_IFBLK) ||
		...
		if (action == 'o') {
			...
			case 0:
				break;
			case EFBIG:
				warnx("ulimit reached for output file.");
				break;
			case ENOSPC:
				warnx("No space left for output file.");
				break;
			default:
				warnx("I/O error - cannot continue, "
						"errno %d, %s",
						err, strerror(err));
			}
		}
	...
	if (err == ENOSPC || err == ENXIO || err == 0)
		fprintf(stderr, sysv3 ? "Reached end of medium on %s.\a\n" :
...
sum(int fd, const char *fn, struct stat *sp, char *tg)
{
	...
	if (rd < 0) {
		warnx("Error computing checksum");
		return 1;
	}
	if (lseek(fd, 0, SEEK_SET) == (off_t)-1) {
		warn("Cannot reset file after checksum");
		return 1;
	}
	char tmp[9];
	sprintf(tmp, "%08lx", (long)sum);
	memcpy(tg, tmp, 8);
	return 0;
}
...
rstime(const char *fn, struct stat *st, const char *which)
{
	...
	if (utime(fn, &utb) < 0) {
	fail:	warn("Unable to reset %s time for \"%s\"", which, fn);
		return 1;
	}
	return 0;
}
...
dopass(const char *target)
{
	...
	if (stat(target, &st) < 0) {
		warn("Error during stat() of \"%s\"", target);
		done(1);
	}
	if ((st.st_mode&S_IFMT) != S_IFDIR) {
		warnx(sysv3 ? "<%s> not a directory." :
				"\"%s\" is not a directory", target);
		done(1);
	}
...
passdata(struct file *f, const char *tgt, int tfd)
{
	...
	while ((rd = read(f->f_fd, buf, bufsize)) > 0) {
		...
		if (write(tfd, buf, rd) != rd) {
			warn("Cannot write \"%s\"", tgt);
			return -1;
		}
	}
	if (rd < 0) {
		warn("Cannot read \"%s\"", f->f_name);
		return -1;
	}
	return 0;
}
...
passfile(const char *fn, struct stat *st)
{
	...
	if ((st->st_mode&S_IFMT) == S_IFLNK) {
		...
		if ((sz = readlink(fn, f.f_lnam, sz+1)) < 0) {
				warn("Cannot read symbolic link \"%s\"", fn);
				free(f.f_lnam);
				return 1;
		}
		f.f_lnam[sz] = '\0';
		...
		if ((f.f_fd = open(fn, O_RDONLY)) < 0) {
			if (sysv3)
				warnx("Cannot open file \"%s\".", fn);
			else
				warn("Cannot open \"%s\", skipped", fn);
			if (st->st_size != 0)
				return 1;
		}
	}
		...
		if (Lflag) {
			...
			for (;;) {
				...
				if ((j = readlink(name, symblink, i)) < 0) {
					warn("Cannot read symbolic link "
						"\"%s\"", name);
					free(symblink);
					if (name != f->f_name)
						free(name);
...
cantlink:	errcnt += 1;
	...
	if ((f->f_st.st_mode&S_IFMT) != S_IFDIR && f->f_st.st_nlink > 1 &&
		...
		if (rename(tgt, temp) < 0) {
			warn("Cannot rename current \"%s\"", tgt);
			tunlink(&temp);
			goto skip;
		}
	}
	...
	case S_IFDIR:
		if (!dflag) {
			if (action == 'p')
				warnx("Use -d option to copy \"%s\"",
						f->f_name);
			goto restore;
		}
		if (mkdir(tgt, 0777) < 0 && errno != EEXIST) {
			warn("Cannot create directory \"%s\"", tgt);
			goto restore;
		}
		break;
	case S_IFLNK:
		if (symlink(f->f_lnam, tgt) < 0) {
			warn("Cannot create \"%s\"", tgt);
			goto restore;
		}
		break;
	case S_IFREG:
	...
	case S_IFNWK:
		if (mknod(tgt, f->f_st.st_mode&(S_IFMT|0777),
					f->f_st.st_rdev) < 0) {
			warn("Cannot mknod() \"%s\"", tgt);
			goto restore;
		}
		break;
	default:
		warnx("Impossible file type");
		goto skip;
	}
	if (vflag)
		fprintf(stderr, "%s\n", f->f_name);
...
restore:
	if (temp) {
		if (rename(temp, tgt) < 0) {
			warn("Cannot recover original version of \"%s\"",
					tgt);
			tunlink(&temp);
		} else
	...
	do {
		...
		if (errno == EEXIST) {
			...
			if (lstat(path1, &st1) == 0 &&
				...
				goto good;
			if (unlink(path2) < 0)
				warn(sysv3 ? "cannot unlink <%s>" :
					"Error cannot unlink \"%s\"", path2);
		}
	} while (twice++ == 0);
	warn(sysv3 ? "Cannot link <%s> & <%s>" :
...
tunlink(char **fn)
{
	...
	if (*fn) {
		if (unlink(*fn) < 0)
			warn("Cannot unlink() temp file \"%s\"", *fn);
		free(*fn);
		*fn = NULL;
	}
}
			...
			*cp = '\0';
			if (stat(fn, &st) < 0) {
				if (dfl) {
					if (mkdir(fn, 0777) < 0) {
						*cp = '/';
						warn("Cannot create directory for \"%s\"",
							fn);
						return -1;
					}
				} else {
...
doinp(void)
{
	...
	if (Iflag) {
		if ((mt = open(Iflag, O_RDONLY)) < 0) {
			if (sysv3) {
				warn("Cannot open <%s> for input.", Iflag);
				done(1);
			}
			else {
				warn("Cannot open \"%s\" for input", Iflag);
...
corrupt:
	if (kflag) {
		if (tflag)
			fflush(stdout);
		warnx("Corrupt header, file(s) may be lost.");
	}
badhdr:
	if (kflag) {
		...
		if (attempts++ == 0) {
			if (tflag)
				fflush(stdout);
			warnx(sysv3 < 0 ? "Out of phase; resyncing." :
	...
	if (tflag)
		fflush(stdout);
	warnx(sysv3 < 0 ? "Out of phase--get help" :
...
again:	if (blktop >= SIZEOF_hdr_cpio &&
		...
		warnx(sysv3 ? "This is not a cpio file, bad header." :
...
again:	while (size) {
	...
	if (fmttype & TYP_CRC) {
		if (f->f_chksum != ssum && f->f_chksum != usum) {
			warnx("\"%s\" - checksum error", f->f_name);
			if (kflag)
				errcnt++;
			else
				val = -1;
...
fail:	warnx("Unable to append to this archive");
	done(1);
	/*NOTREACHED*/
	return 1;
}
...
unexeoa(void)
{
	if (sysv3) {
		...
		warnx("Unexpected end-of-archive encountered.");
	done(1);
}
...
{
	if (fstat(mt, &mtst) < 0) {
		warn("Error during stat() of archive");
		done(1);
	}
...
want(struct file *f, struct glist **gb)
{
	...
	for (gp = patterns; gp; gp = gp->g_nxt) {
		if ((fnmatch(gp->g_pat, f->f_name, 0) == 0) ^ gp->g_not &&
...
patfile(void)
{
	...
	if ((f = fopen(Eflag, "rb")) == NULL) {
		warn("Cannot open \"%s\" to read patterns", Eflag);
		usage();
	}
	while ((namlen = getline(&name, &namsiz, f)) != 0) {
		if (name[namlen-1] == '\n')
			name[--namlen] = '\0';
		addg(name, 0);
	}
...
{
	if (size % mod) {
		warnx("Cannot swap %s of \"%s\", odd number of %s",
				str, fn, str);
		errcnt++;
		return 1;
	}
...
tmklink(struct tar_header *hp, const char *fn)
{
	...
	if (fmttype == FMT_GNUTAR && cp - fn > 99) {
		...
		warnx("%s: linked name too long", fn);
		return -1;
	}
...
trdsum(union bincpio *bp)
{
	...
	if (i)
		warnx("Bad header - checksum error.");
	return i;
}
...
{
	if (sysv3)
		warnx("format can't support expanded types on %s", file);
	else
		warnx("%s format can't support expanded types on %s",
...
onint(int signo)
{
	if (cur_ofile && cur_tfile && rename(cur_tfile, cur_ofile) < 0)
		warn("Cannot recover original \"%s\"", cur_ofile);
	if (cur_ofile && cur_tfile == NULL && unlink(cur_ofile) < 0)
		warn("Cannot remove incomplete \"%s\"", cur_ofile);
	exit(signo | 0200);
}
...
zipread(struct file *f, const char *tgt, int tfd, int doswap)
{
	...
	if (f->f_gflag & FG_DESC) {
		if (f->f_cmethod != C_DEFLATED && f->f_cmethod != C_ENHDEFLD ||
				f->f_gflag & FG_CRYPT) {
			warnx("Cannot handle zip data descriptor");
			done(1);
		}
...
zipreaddesc(struct file *f)
{
	if (f->f_oflag & OF_ZIP64) {
		...
		if (memcmp(zd64.zd_signature, mag_zipdds, sizeof mag_zipdds)) {
			warnx("Invalid zip data descriptor");
			done(1);
		}
...
{
	warnx("Cannot unzip %s file \"%s\"", method, f->f_name);
	errcnt++;
	return skipfile(f);
}
...
zipunbz2(struct file *f, const char *tgt, int tfd, int doswap, uint32_t *crc)
{
	...
	if ((ok = BZ2_bzDecompressInit(&bs, 0, 0)) != BZ_OK) {
		warnx("bzip2 initialization error %d on \"%s\"",
				ok, f->f_name);
		errcnt++;
		return skipfile(f);
	}
		...
		if (ok == BZ_OK) {
			...
			case BZ_STREAM_END:
				...
				*crc = zipcrc(*crc, (unsigned char *)obuf, on);
				break;
			default:
				warnx("compression error %d on \"%s\"",
					ok, f->f_name);
				errcnt++;
				val = 1;
			}
...
zipblast(struct file *f, const char *tgt, int tfd, int doswap, uint32_t *crc)
{
	...
	case 0:
		break;
	default:
		warnx("compression error %d on \"%s\"", n, f->f_name);
		errcnt++;
		bh.bh_val = 1;
	}
	while (bh.bh_isize) {
...
nextfield(char *cp, const char *fieldname)
{
	while (*cp && *cp != ':')
		cp++;
	if (*cp == 0) {
		warnx("line %lld: unterminated \"%s\" field",
				lineno, fieldname);
		done(1);
	}
	*cp++ = 0;
...
getproto(char *np, struct prototype *pp)
{
	...
	if (*np == ':')
		...
		if (*xp == '\0')
			...
			if ((grp = getgrnam(np)) == NULL) {
				warnx("line %lld: unknown group \"%s\"",
						lineno, np);
				done(1);
			}
	...
	if (*np == ':') {
		np++;
		if (*np++ != ':') {
		majmin:	warnx("line %lld: need either both major and "
				"minor or none",
				lineno);
			done(1);
		}
		...
		t = strtoll(np, &xp, 10);
		if (*xp) {
			warnx("line %lld: illegal major \"%s\"",
					lineno, np);
			done(1);
		}
		np = tp;
		...
		u = strtoll(np, &xp, 10);
		if (*xp) {
			warnx("line %lld: illegal minor \"%s\"",
					lineno, np);
			done(1);
		}
		np = tp;
